...
28. **参数分层 + 变体预烘（2026-09-13，用户拍板）**：游戏内可调性分三档——① **行为参数热调**（L0/L1/L2 分层开关、每件 L2 开关、L1 W/fade/release-cancel、延音上限、音量覆盖）：改完下一音符生效；② **样本参数档位热切（变体预烘）**：M2a builder 导出资源包时除调优值外，预烘「平台长度平坦区相邻档（与最优点 worst_pp 差 ≤2pp 才烘，含调优值共 ≤3 档）+ L2 六件 xfade 5ms/40ms 双档 + 16 件未整形原版（单件回原版开关）」，全部变体在同一资源包内常驻内存 → 游戏内下拉切换事件指向即生效，**无需 F3+T**（包体 +<1MB）；③ **任意连续值** = 运行时 DSP（Java 移植 l0shape 对原版解码缓冲重整形），列为 M4+ 高级模式，不进 M1。M1 配置三 Tab：①图层开关（含每件变体下拉）②L1 参数 ③高级（延音上限/音量/调试 overlay）。行为类热生效、变体切换热生效、换资源包/整套参数才需 F3+T。**开关粒度 = 乐器种类级（用户拍板）**：全局 L0/L1/L2 总开关 + 16 件乐器各自 {l0,l1,l2,variant} 开关，不做按方块坐标覆盖；默认值按路由表（7 件 L2=全开、L2 不可行旋律件=仅 L0、打击乐=仅 L0(48ms)+L1 关、全关组合=原版原样 A/B 对照）；schema 见 docs/06 §4。**运行时 DSP 架构已查通 [1.21.11 已验证]**：1.21.9+ 重构后 PCM 在 `com.mojang.blaze3d.audio.SoundBuffer`(fwl) 的 ByteBuffer 中、**懒上传** OpenAL（首播 alBufferData 后 CPU 副本置 null）→ `Library.init` 后首播前是换缓冲窗口；`Sound`(ipj) 只持事件+音高音量、旧 `OggData` 已废；`SoundEngine`=iqo@net.minecraft.client.sounds、`SoundEvent`=bcz@net.minecraft.sounds。
29. **M2a 改道：运行时 DSP 为核心（2026-09-13，用户拍板 A）**：用户选 A（贴合"实时计算"倾向）——**M2a = Java DSP 核心**：① `L0Shaper`（l0shape.py 数学移植：截尾平台/sin² 淡出/包络压平/DC 移除/峰值归一，含打击乐 48+4ms 规格）② `LoopDetector`（loopdetect v5 移植：find_loop NCC 搜索 + seamErr 诊断 + keep-best xfade）③ MC 挂点 = `Library.init` 后/`SoundBuffer.a()` 上传前对 16 件 note 缓冲重整形（保留原始 ByteBuffer 副本，改参=从原始重整形换入，热生效；F3+T 重应用）④ **交叉核对测试**（守 Hy4 #12 纪律）：16 件原版 WAV 夹具，Java 输出 vs Python l0shape 输出 max|diff| 容差内一致，JUnit 跑通才算移植合格。资源包 builder（变体预烘）降为**可选兼容件**（无模组场景/单包分发），不再是主路径。L2 延音随之改为**运行时 loop 缓冲 + AL 循环点**（M4），不再烘长 ogg。里程碑重排：M1 配置 → M2a Java DSP 核心+核对 → M3 L1 → M4 L2（运行时 loop）→ M5 26.x → M6。
30. **M2a Phase 1 完成（2026-09-13，`5f11751`+`d3e8d7e`）**：①②④ 全部落地并 JUnit 绿：`L0Shaper`（16/16 maxDiff=**0.00e+00** 位级一致）；`LoopDetector`（loopdetect v5 全管线移植：preprocess/est_f0+子谐波/renv/nfl_plateau/find_loop NCC 搜索/build_loop_diag+keep-best xfade/render/metrics/seam_err_cycles/v5 verdict；`FFT` = radix-2 + 精确 DFT 双实现）；交叉核对 = **9 OK 字段级一致（a/L/k 精确、f0±0.01Hz、NCC±1e-3、dB 字段 ±0.05、verdict 精确）+ 7 REJECT 原因字符串一致**，参考参数 = v5 规范（ncc 0.95/span 0.5）+ didgeridoo 定参（ncc 0.90/span 1.0），与 result-v5-vanilla12111 逐项对齐。夹具 = `tools/gen_dsp_fixtures.py`（WAV32F 输入 + `.ref` float32 + `fixtures.json` + `loop-ref.json`，改定参后需重跑）。**移植坑（防复现）**：① Python `round()` 银行家舍入 → Java 用 `Math.rint`（`Math.round` 在 220.5 差 1 样本，影响 44.1k 件 xfade 长度）；② `np.linspace(0,1,N)` 的 N=1 时 = [0.0]（非 1.0）；③ NCC 双窗口端点必须都带 `a` 基准（`xf[a:a+Ws]` / `xf[a+L:a+L+Ws]`）——端点基准写错只在贴边案例（banjo，NCC 余量 0.0008）显形，15 件候选远离阈值不暴露；④ Java radix-2 FFT 与 numpy pocketfft 低 10-12 位小数差异 → 若未来某件 NCC 落在 0.95±1e-6 路由可能两边不一致（16 件最小余量 0.0008，无影响）。**下一步 = M2a ③ MC 挂点**（SoundBuffer 懒上传窗口换缓冲 + 原始副本常驻 + 改参热重整形）。
31. **M2a ③ MC 挂点完成（2026-09-13，实机验证绿）**：运行时 DSP 挂点落地并通过真实 80 模组环境（1.21.11，HMCL）实机验证：
    - **架构（最终）**：接管 = `SoundBufferLibrary.getCompleteBuffer` **@Overwrite**（原体逐行重实现：`cache.computeIfAbsent` + `JOrbisAudioStream.readAll()/getFormat()` + `Util.nonCriticalIoPool()`；私有字段经 `SoundBufferLibraryAccessor` @Accessor 接口）后把返回 future 包一层 `thenApply` → 接管回调成为原 future 依赖节点、播放方回调挂包装 future → **结构上保证首整形先于首次上传**（缓冲刚解码、data 必可读、无竞态）。上传前兜底 = `SoundBuffer.getAlBuffer` HEAD 注入：配置代际过期 → discardAlBuffer + 从原始重整形 + 换 data（热改参下一音符生效，AGENTS #28）。状态 = `WeakHashMap<SoundBuffer, State{original float[], AudioFormat, instrument}>`（原始 PCM 常驻，M4 L2 依赖）；`clear()`（F3+T/重载）释放状态、新缓冲自动重接管。
    - **实机验证**：16/16 件全部接管（含 (Ballad|谣) 包 5 件 **16bit stereo 44.1k** → 平均降混单声道、回写双声道复制）；端到端交叉核对 `tools/verify_dsp_dump_ingame.py`（游戏内解码 PCM → Python l0shape 同参 vs Java shaped，float32 转储 `DEV_DUMP`）**16/16 maxDiff≈3e-8**、长度=平台规格；JUnit 回归仍 16/16 位级。
    - **挂点移植坑（全部实测，防复现）**：① `getCompleteBuffer` 单表达式尾返回 → RETURN 注入点**不可取消**（`setReturnValue` 抛 `CancellationException`）；② 本模组环境（replaymod 内置 nested `mixinextras-fabric-0.3.6`）中**任何** `@Redirect` 都会 CCE：其 `FactoryRedirectWrapperMixinTransformer` 把 `at`（bytecode 恒为数组值）强转 `AnnotationNode`，只认 `at=@At("NEW")` 工厂形态（`Annotations.getValue` 原样返回 ArrayList）→ 该环境下 @Redirect 整体不可用；③ 同方法 @Invoker+@Overwrite = 互递归 StackOverflow（invoker 委托到已被 Overwrite 的方法名）；④ 类内 `private abstract` 是 JDK21 preview → Accessor/Invoker 一律放接口 mixin；⑤ WAV float32 数据必须小端（`DataOutputStream.writeInt` 是大端 → 读回 1e-38/1e38 次正规噪声，20:5x 实测）；⑥ `Sound.getPath()` = `sounds/note/<name>.ogg`（`FileToIdConverter("sounds",".ogg")` 前缀，sounds.json 条目 `note/harp2` → 路径 `sounds/note/harp2.ogg`）。
...
32. **M1 配置 dev 冒烟绿 + cloth-config bundle 四大坑（2026-09-14，loader 0.19.5 / loom 1.17.20 反编译实查）**：
    - **版本钉**：cloth-config = **21.11.153+fabric**（Modrinth 2026-09-13 查为 1.21.11 最新稳定，LGPLv3，SHA512 已验；单 jar 内含 nested `basic-math-0.6.1.jar`（mod id `cloth-basic-math`））；ModMenu 17.0.0 = `modCompileOnly`（可选增强，用户环境为 17.0.0-beta.1）。
    ...
    - **实机冒烟绿（用户，2026-09-14，HMCL 实例 1.21.11，17 模组）**：配置界面（自动弹出 + ModMenu 入口）/ 键绑 / 热改生效 / 16 件接管 **全部绿灯**（用户确认「全部测试绿灯」）。
    - **下一步 = M3 L1**（2gt 合并窗口 + release-cancel 语音合并状态机）。
...
- **实机验证实例（HMCL，1.21.11）**：`C:\01_Self_Software\Minecraft\.minecraft`（含 Ballad Plus 资源包；**17 模组**：replaymod 2.6.25 / modmenu 17.0.0-beta.1 / iris 1.10.4 / sodium 0.8.2 / lithium 0.21.2 / voxy 0.2.8 / litematica 0.25.3（+malilib 0.27.2）/ xaerominimap 25.3.5 + xaeroworldmap 1.40.6 / worldedit 7.4.0-beta02（+WorldEditCUI）/ AnotherColorBlock / placeholder-api / stringduperfix；**fabric-api = 0.140.2+1.21.11**，我们 depends `*` 兼容）。**2026-09-14 全扫（17 个 jar 的 fabric.mod.json + 类条目）**：无模组声明 cloth-config 依赖、无模组内嵌 `me.shedaniel.*` 类 → 我们 bundled cloth **无 mod-id/类重复冲突**；本环境唯一已确认的跨模组冲突 = replaymod 内置 nested `mixinextras-fabric-0.3.6` 致 `@Redirect` 不可用（见 #31 ②）。
