...
输出:
    attackEnd / loopStart / loopEnd  (单位: 样本 @ 44.1kHz)
"""
...
def analyze(path, args):
    ...
    if sr != SR:
        x = resample(x, int(len(x) * SR / sr)) if x.ndim == 1 else \
            np.stack([resample(x[:, i], int(len(x) * SR / sr)) for i in range(x.shape[1])], 1)
    x = preprocess(x)
