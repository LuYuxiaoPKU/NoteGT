...
def seam_ratio_lf(loop, f0, cutoff_mul=1.5):
    """低频(基频)接缝连续性：LP 到 1.5*f0 后再量跳变  [DEPRECATED 第四轮]

    两个独立缺陷（Hy4 2026-08 第四轮诊断）：
    1) filtfilt 默认 padlen=9 样本，而该滤波器冲激响应长达数百样本 → 边缘瞬态未被吸收，
       完美周期循环也给出 61~224 的比值（∝ 1/f0，量的是滤波器病态，不是接缝）；
    2) 低通把咔哒抹成斜坡 → 测不到跳变；半周期错位反而"更好"。
    保留仅为向后兼容，判读请改用 seam_click_db + seam_phase_deg。
...
def seam_click_db(loop):
    """未滤波：接缝跳变 / 信号 RMS。答'有没有咔哒'。（Hy4 第四轮新增）
    判读：<= -40 dB 安全；-30 边界；>= -25 可闻（masking 启发值，非实测听阈）。
    """
    rms = float(np.sqrt(np.mean(loop**2)))
    return 20*np.log10(max(abs(loop[0]-loop[-1]),1e-12)/max(rms,1e-12))


def seam_phase_deg(loop, f0, reps=7):
    """循环平铺后低通+Hilbert 相位：答'基频在接缝处连不连续'。（Hy4 第四轮新增）
    平铺消除 filtfilt 边缘瞬态（修复 seam_ratio_lf 的 padlen 病态）。
    判读：< 10° 好；> 45° 需修（接缝处基频凹陷）。
    """
    from scipy.signal import butter, sosfiltfilt, hilbert
    wc = min(1.5*f0/(SR/2), 0.9)
    if wc <= 0:
        return float('nan')
    sos = butter(4, wc, 'low', output='sos')
    n = len(loop)
    Y = sosfiltfilt(sos, np.tile(loop, reps))     # 平铺消除边缘瞬态
    y = Y[(reps//2)*n:(reps//2+1)*n]
    ph = np.unwrap(np.angle(hilbert(y)))
    d = ph[0] - ph[-1] - 2*np.pi*f0/SR
    return np.degrees((d+np.pi) % (2*np.pi) - np.pi)
...
def analyze(path, args):
    ...
    # 自适应兜底：接缝不达标就加大循环点淡化（第四轮修复：保留最优而非最后一个，
    # 仅在真正改善 sm < best_sm - 0.05 时替换，避免无改善时留下额外音色涂抹）
    ...
    for xf in (0.010, 0.020, 0.040):
        ...
        if len(alt) == len(loop):
            alt_sm = seam_ratio(alt)
            if alt_sm < best_sm - 0.05:
                loop = alt
                best_sm = alt_sm
    ...
    res = dict(file=path, ok=True,
               ...
               seamFundRatio=round(smlf, 2) if smlf == smlf else None,  # deprecated（第四轮，仅向后兼容）
                seamClickDb=round(seam_click_db(loop), 2),
                seamPhaseDeg=round(seam_phase_deg(loop, f0), 2) if f0 > 0 else None,
