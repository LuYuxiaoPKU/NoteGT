# minecraft-mod-dev SKILL.md（github.com/chouzz/minecraft-mod-dev 原版）

> 来源：https://github.com/chouzz/minecraft-mod-dev（master @ 2026-08-07，17 stars）
> 描述：Claude Code skill for Minecraft mod development with NeoForge/Fabric support and inter-mod integration (JEI/AE2/Create)
> 文件：skills/minecraft-mod-dev/SKILL.md + references/migration-guide.md（独立存档）+ references/mod-links.md + scripts/mod-env-check.sh
...
- **Create:** Leverage `Create`'s `AllBlockEntityType` patterns or custom `mixing/pressing` recipe types.
- **Convenience Tags:** Use `#c:ingots`, `#c:dusts` (for 1.21 NeoForge) to ensure cross-mod compatibility.
